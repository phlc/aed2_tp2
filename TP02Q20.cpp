...
/**
*ordenarInsercaoParcial - ordena parcialmente metodo insercao
*@param Lista*, int k,  int log[]
*/
void ordenarInsercaoParcial (Lista* p_lista, int k, int log[]){
	for (int i=1; i<p_lista->fim; i++){
		int j = i, ref;
		...
		log[0]++;
		while(j>0 && compare(p_lista->list[ref], p_lista->list[j], 6)>0.0){
			log[0]++;
			log[1]+=3;
			swap(p_lista, ref, j);
			j=ref;
...
int  main(void){
	...
	ordenarInsercaoParcial(p_lista, 10,  log);
	clock_t fim = clock();
	...
	//arquivo log
	FILE* arq = fopen("651230_insercaoParcial.txt", "wt");
