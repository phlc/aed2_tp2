...
/**
*ordenarBubbleSort - ordena metodo quicksort
*@param Lista*, int log[]
*/
void ordenarBubbleSort (Lista* p_lista, int log[]){
	for (int i=1; i< p_lista->fim; i++){
		for (int j=p_lista->fim-1; j>=i; j--){
			log[0]++;
			if (compare(p_lista->list[j-1], p_lista->list[j], 6)>0.0){
				log[1]+=3;
				swap(p_lista, j-1, j);
			}
		}
...
int  main(void){
	...
	//arquivo log
	FILE* arq = fopen("651230_bolha.txt", "wt");
